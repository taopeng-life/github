...
class Databases:
    ...
    def getCurrentDb(self):
        ...
        if Backend.getIdentifiedDbms() in (DBMS.ORACLE, DBMS.DB2, DBMS.PGSQL):
            warnMsg = "在%s上，您需要使用枚举的模式名作为" % Backend.getIdentifiedDbms()
            warnMsg += "对应于其他DBMS的数据库名称"
    ...
    def getDbs(self):
        ...
        if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
            warnMsg = u"information_schema不可用，后端DBMS是MySQL<5.X版本的，数据库名称将从“mysql”数据库中获取。"
            logger.warn(warnMsg)
    ...
    def getTables(self, bruteForce=None):
        ...
        if bruteForce:
            ...
            message = u"您是否希望使用公共表检查存在性? %s " % ("[Y/n/q]" if Backend.getIdentifiedDbms() in (DBMS.ACCESS,) else "[y/N/q]")
            ...
            if condition:
                if not Backend.isDbms(DBMS.SQLITE):
                    ...
                    if conf.excludeSysDbs:
                        infoMsg = u"跳过系统数据库%s '%s'" % ("s" if len(self.excludeDbsList) > 1 else "", ", ".join(unsafeSQLIdentificatorNaming(db) for db in self.excludeDbsList))
                        logger.info(infoMsg)
        ...
        if not kb.data.cachedTables and isInferenceAvailable() and not conf.direct:
            for db in dbs:
                ...
                infoMsg = u"获取数据库'%s'中的表数量" % unsafeSQLIdentificatorNaming(db)
                logger.info(infoMsg)
                ...
                if count == 0:
                    warnMsg = u"数据库'%s'似乎是空的" % unsafeSQLIdentificatorNaming(db)
                    logger.warn(warnMsg)
                    continue

                elif not isNumPosStrValue(count):
                    warnMsg = u"无法检索数据库'%s'的表数量" % unsafeSQLIdentificatorNaming(db)
                    logger.warn(warnMsg)
                    continue
                ...
                if tables:
                    ...
                    warnMsg = u"无法检索数据库'%s'的表名" % unsafeSQLIdentificatorNaming(db)
                    logger.warn(warnMsg)
    ...
    def getColumns(self, onlyColNames=False, colTuple=None, bruteForce=None, dumpMode=False):
        ...
        if conf.db is None or conf.db == CURRENT_DB:
            if conf.db is None:
                warnMsg = u"缺少数据库参数,sqlmap将使用当前数据库枚举表中的字段"
                logger.warn(warnMsg)

            conf.db = self.getCurrentDb()

            if not conf.db:
                errMsg = u"无法检索当前数据库名称"
                raise SqlmapNoneDataException(errMsg)
        ...
        if conf.tbl:
            ...
            if len(kb.data.cachedTables) > 0:
                ...
                errMsg = u"无法检索数据库'%s'中的表" % unsafeSQLIdentificatorNaming(conf.db)
                raise SqlmapNoneDataException(errMsg)
            else:
        ...
        if bruteForce is None:
            if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
                errMsg = u"information_schema不可用，后端DBMS是MySQL < 5.0"
                logger.error(errMsg)
                bruteForce = True

            elif Backend.isDbms(DBMS.ACCESS):
                errMsg = u"无法检索列名，后端DBMS为 %s" % DBMS.ACCESS
                logger.error(errMsg)
                bruteForce = True
            ...
            for tbl in tblList:
                if conf.db is not None and len(kb.data.cachedColumns) > 0 \
                   ...
                   kb.data.cachedColumns[conf.db]:
                    infoMsg = u"在数据库'%s'上获取表的字段" % unsafeSQLIdentificatorNaming(conf.db)
                    logger.info(infoMsg)
    ...
    def getCount(self):
        if not conf.tbl:
            warnMsg = "缺少表参数，sqlmap将检索所有数据库管理系统数据库表的条目数"
            logger.warn(warnMsg)
        ...
        if conf.tbl is not None and conf.db is None and Backend.getIdentifiedDbms() not in (DBMS.SQLITE, DBMS.ACCESS, DBMS.FIREBIRD):
            warnMsg = "缺少数据库参数.sqlmap将使用当前数据库来检索表 '%s'的条目数" % unsafeSQLIdentificatorNaming(conf.tbl)
            logger.warn(warnMsg)
