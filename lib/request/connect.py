...
class Connect(object):
    ...
    def _retryProxy(**kwargs):
        ...
        if kb.testMode and kb.previousMethod == PAYLOAD.METHOD.TIME:
            # 基于定时的有效载荷可能导致Web服务器无响应，如果可注入的代码段是某种类似JOIN的查询
            warnMsg = u"最有可能注入的Web服务器实例还没有从以前基于定时的有效载荷中恢复。"
            warnMsg += u"如果问题仍然存在，请等待几分钟，并在选项'--technique' "
            warnMsg += u"（例如“--flush-session --technique = BEUS”）中重新运行“T”标志，"
            warnMsg += u"或尝试降低选项'time-sec'的参数值(例如'--time-sec = 2'降低延迟注入的时间)"
            singleTimeWarnMessage(warnMsg)

        elif kb.originalPage is None:
            if conf.tor:
                warnMsg = u"请确保您已经安装并运行Tor，以便您可以成功使用选项'--tor'"
                if IS_WIN:
                    warnMsg += u"(例如 'https://www.torproject.org/download/download.html.en')"
                else:
                    warnMsg += u"(例如 'https://help.ubuntu.com/community/Tor')"
            else:
                warnMsg = u"如果问题仍然存在，请检查提供的目标网址是否有效。 "
                warnMsg += u"\r\n您可以尝试使用“--random-agent”选项和设置代理(“--ignore-proxy”,“--proxy”)"
            singleTimeWarnMessage(warnMsg)

        elif conf.threads > 1:
            warnMsg = u"如果问题仍然存在，请尝试降低使用的线程数(选项 '--threads')"
            singleTimeWarnMessage(warnMsg)
    ...
    def getPage(**kwargs):
        ...
        # 支持非拉丁(例如西里尔语)URL，因为urllib/urllib2不支持默认URL
        url = asciifyUrl(url)
        ...
        try:
            ...
            if headers.get(HTTP_HEADER.CONTENT_TYPE) == POST_HINT_CONTENT_TYPES[POST_HINT.MULTIPART]:
                warnMsg = u"在'%s'标题中缺少'边界参数'" % HTTP_HEADER.CONTENT_TYPE
                warnMsg += u"将尝试重构"
                singleTimeWarnMessage(warnMsg)
            ...
            if websocket_:
                ...
                ws.connect(url, header=("%s: %s" % _ for _ in headers.items() if _[0] not in ("Host",)), cookie=cookie)  # WebSocket将自动添加主机字段的头
            ...
            if not refreshing:
                ...
                if extractRegexResult(META_REFRESH_REGEX, page):
                    ...
                    debugMsg = u"获得 HTML meta refresh header"
                    logger.debug(debugMsg)

                if refresh:
                    if kb.alwaysRefresh is None:
                        msg = u"sqlmap得到刷新请求(类似于登录页面的重定向响应),"
                        msg += u"要从现在开始刷新还是留在原始页面? [Y/n]"
            ...
            if conn and not conf.keepAlive:
                try:
                    ...
                    warnMsg = u"连接关闭期间发生问题('%s')" % getSafeExString(ex)
                    logger.warn(warnMsg)
            ...
            try:
                ...
                warnMsg = u"连接超时，同时尝试获取错误页面信息(%d)" % ex.code
                logger.warn(warnMsg)
                return None, None, None
            ...
            if ex.code == httplib.UNAUTHORIZED and not conf.ignore401:
                errMsg = u"未授权，尝试提供正确的HTTP身份验证类型和有效证书(%d)" % code
                raise SqlmapConnectionException(errMsg)
            elif ex.code == httplib.NOT_FOUND:
                if raise404:
                    errMsg = u"找不到页面 (%d)" % code
                    raise SqlmapConnectionException(errMsg)
                else:
                    debugMsg = u"找不到页面 (%d)" % code
                    singleTimeLogMessage(debugMsg, logging.DEBUG)
            elif ex.code == httplib.GATEWAY_TIMEOUT:
                if ignoreTimeout:
                    return None if not conf.ignoreTimeouts else "", None, None
                else:
                    warnMsg = u"无法连接到目标网址 (%d - %s)" % (ex.code, httplib.responses[ex.code])
                    if threadData.retriesCount < conf.retries and not kb.threadException:
                        warnMsg += u"，sqlmap将重试该请求"
                ...
                debugMsg = u"收到HTTP错误代码: %d (%s)" % (code, status)
                logger.debug(debugMsg)
            ...
            if checking:
                ...
                if kb.testMode and kb.testType not in (None, PAYLOAD.TECHNIQUE.TIME, PAYLOAD.TECHNIQUE.STACKED):
                    singleTimeWarnMessage(u"目标(或WAF/IPS/IDS)可能会丢弃“可疑”的请求")
                    kb.droppingRequests = True
                warnMsg = u"连接超时到目标网址"
            elif "Connection reset" in tbMsg:
                if not conf.disablePrecon:
                    singleTimeWarnMessage(u"由于连接复位而关闭预连接机制")
                    conf.disablePrecon = True

                if kb.testMode:
                    singleTimeWarnMessage(u"目标(或WAF/IPS/IDS)有可能重置“可疑”请求")
                    kb.droppingRequests = True
                warnMsg = u"连接重置到目标URL"
                ...
                warnMsg = u"NTLM认证出现问题"
            elif "Invalid header name" in tbMsg:  # (e.g. PostgreSQL ::Text payload)
                return None, None, None
            elif "BadStatusLine" in tbMsg:
                warnMsg = u"连接丢失或收到未知HTTP状态代码"
                if not conf.agent and not conf.randomAgent:
                    warnMsg += u"。尝试强制使用选项'--user-agent'的HTTP User-Agent头，"
                    warnMsg += u"或者使用选项'--random-agent'提供的随机HTTP User-Agent头"
            elif "IncompleteRead" in tbMsg:
                warnMsg = u"从目标URL检索数据时出现了一个不完整的读取错误"
            elif "Handshake status" in tbMsg:
                status = re.search("Handshake status ([\d]{3})", tbMsg)
                errMsg = u"websocket握手状态%s" % status.group(1) if status else "unknown"
                raise SqlmapConnectionException(errMsg)
            else:
                warnMsg = u"无法连接到目标网址"
            ...
            with kb.locks.connError:
                ...
                if kb.connErrorCounter >= MAX_CONSECUTIVE_CONNECTION_ERRORS and kb.connErrorChoice is None:
                    message = u"与目标网址连续连接中，"
                    message += u"你确定要继续对目标进行测试吗? [y/N] "
    ...
    def queryPage(value=None, place=None, content=False, getRatioValue=False, silent=False, method=None, timeBasedCompare=False, noteResponseTime=True, auxHeaders=None, response=False, raise404=None, removeReflection=True):
        """
        此方法调用一个函数来获取目标URL页面内容，
        并返回其页面MD5散列值或布尔值，以防字符串匹配检查（'--string'命令行参数）
        """
            ...
            if kb.tamperFunctions:
                for function in kb.tamperFunctions:
                    try:
                        ...
                        errMsg = u"运行'%s'tamper脚本时发生错误('%s')" % (function.func_name, getSafeExString(ex))
        ...
        if timeBasedCompare and not conf.disableStats:
            ...
            if not kb.laggingChecked:
                ...
                if deviation > WARN_TIME_STDEV:
                    ...
                    warnMsg = u"在连接响应中已经检测到相当大的滞后，"
                    warnMsg += u"请尽可能选择'--time-sec' (如10以上)"
                    logger.critical(warnMsg)
